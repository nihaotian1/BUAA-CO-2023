...
def cpu_status_init():
    ...
    pc += 4 * 3
    # start interrupt_EN
    init_asm.append('ori $1, 0xff01\n')
    init_asm.append('mtc0 $1, $12\n')
    init_asm.append('lui $1, 0x0000\n')

    if config['start_timer']:  # ctrl : 011 001 time_cnt = 0x01ff
        pc += 4 * 7
        init_asm.append('ori $1, 0x00ff\n')
        init_asm.append('ori $2, 0x0009\n')
        init_asm.append('ori $3, 0x000b\n')

        init_asm.append('sw $1, %d($0)\n' % 0x7f04)
        init_asm.append('sw $1, %d($0)\n' % 0x7f14)

        init_asm.append('sw $2, %d($0)\n' % 0x7f00)
        init_asm.append('sw $3, %d($0)\n' % 0x7f10)
...
def generate_instruction_P6():
    ...
    for i in range(config['test_size']):
        ...
        if suit == 5:  # is jal, we should write db, ins1, ins2, jr
            ...
            # jumps.append(jr(rs, rt, rd))
            how_to_jump = random.randint(0, 1)
            if how_to_jump == 0:
                jumps.append(jr(rs, rt, rd))
            else:
                ins = "mtc0 $31, $14\neret\n"
